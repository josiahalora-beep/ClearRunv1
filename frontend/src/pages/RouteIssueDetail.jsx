...
function loadDemoState(issue) {
  ...
  try {
    const saved = window.localStorage.getItem(`clearrun-route-issue-${issue.id}`);
...
export default function RouteIssueDetail({ issue }) {
  const [demoState, setDemoState] = useState(() => loadDemoState(issue));

  useEffect(() => {
    if (typeof window === "undefined") return;
    window.localStorage.setItem(`clearrun-route-issue-${issue.id}`, JSON.stringify(demoState));
  }, [demoState, issue.id]);

  const allRequirementsComplete = useMemo(
    () => issue.proofNeeded.every((item) => demoState.proofChecks[item]),
    [demoState.proofChecks, issue.proofNeeded]
  );
  const readyToClose = Boolean(demoState.owner) && allRequirementsComplete;
  const currentStatus = demoState.released ? "Released" : issue.status;
  ...
  const handleOwnerChange = (event) => {
    ...
    setDemoState((current) => ({
      ...
      owner,
      activity: [
        { id: `${Date.now()}-owner`, label: `Assigned to ${owner}`, note: "Assignment updated in this browser demo." },
        ...current.activity,
      ],
    }));
  };

  const handleRequirementToggle = (item) => {
      ...
      activity: [
        {
          ...
          note: nextValue ? "Closeout requirement marked complete." : "This item needs review again.",
        },
        ...current.activity,
      ],
    }));
  ...
  const handleClose = () => {
    if (!readyToClose || demoState.released) return;
    setDemoState((current) => ({
      ...current,
      released: true,
      activity: [
        {
          id: `${Date.now()}-closed`,
          label: "Issue marked complete",
          note: "All sample closeout requirements were confirmed.",
        },
        ...current.activity,
      ],
    }));
  ...
  const handleReset = () => {
    ...
    setDemoState(loadDemoState({ ...issue, id: `${issue.id}-reset` }));
  };
    ...
    <Layout>
      <main className="container-page py-10 sm:py-14" data-testid="route-exception-detail">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-base leading-7 text-slate-600">
              Review the route details, assign the follow-up, confirm what is still needed, and close the issue when the ticket is ready.
            </p>
          </div>
                ...
                <div className="bg-ink p-6 text-white sm:p-8">
                  ...
                  <div className="mt-6 grid gap-3 sm:grid-cols-2">
                    {[
                      ...
                      ["Customer contacted", issue.customerNotification],
                    ].map(([label, value]) => (
                ...
                <div className="p-5 sm:p-6">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <label htmlFor="route-issue-owner" className="text-xs font-semibold uppercase tracking-wide text-slate-500">Assigned to</label>
                      <select
                        id="route-issue-owner"
                  ...
                  <div className="mt-5 rounded-2xl border border-slate-200 bg-offwhite p-4">
                    <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Before this can close</p>
                    <p className="mt-2 text-sm font-semibold leading-6 text-navy-950">{issue.releaseCondition}</p>
                  </div>

                  <div className="mt-5">
                    <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Still needs to be confirmed</p>
            ...
            <section className="rounded-premium border border-slate-200 bg-white p-5 shadow-card" data-testid="route-exception-context">
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {[
                  ...
                  ["Truck", issue.truck],
                  ["Driver", issue.technician],
                  ["Reported", issue.reportedTime],
                  ["Ticket backup", issue.evidenceSummary],
                  ["Disposal receipt", issue.disposalStatus],
                ].map(([label, value]) => (
            ...
            <section className="rounded-premium border border-slate-200 bg-white p-5 shadow-editorial" data-testid="route-exception-followup">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <h2 className="mt-1 font-display text-2xl font-semibold text-navy-950">Prepare the follow-up</h2>
                </div>
              </div>
              <textarea
              ...
              <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <p className="text-xs leading-5 text-slate-500">Sample only. This saves the prepared follow-up in this browser and does not send a real message.</p>
                <Button type="button" onClick={handlePrepareFollowUp} data-testid="route-exception-followup-button">
                  {demoState.followUpPrepared ? "Follow-Up Prepared" : "Prepare Follow-Up"}
                </Button>
              </div>
            </section>
          </div>
          ...
          <aside className="space-y-6">
            <section className="premium-card-dark">
              ...
              <h2 className="mt-3 font-display text-3xl font-semibold leading-tight text-white">
                {demoState.released ? "Issue complete." : readyToClose ? "Ready to mark complete." : "More work is needed."}
              </h2>
              <p className="mt-3 text-sm leading-6 text-white/65">
                Confirm who owns the follow-up and complete every required item before marking this issue done.
              </p>
              <div className="mt-6 grid gap-2">
                <div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-white/10 px-3 py-3 text-sm font-semibold text-white/80">
                  <CheckCircle2 className={`h-4 w-4 ${demoState.owner ? "text-white" : "text-white/30"}`} aria-hidden="true" /> Follow-up assigned
                </div>
                <div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-white/10 px-3 py-3 text-sm font-semibold text-white/80">
                  <CheckCircle2 className={`h-4 w-4 ${allRequirementsComplete ? "text-white" : "text-white/30"}`} aria-hidden="true" /> Required items confirmed
                </div>
                <div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-white/10 px-3 py-3 text-sm font-semibold text-white/80">
              ...
              <Button
                type="button"
                onClick={handleClose}
                disabled={!readyToClose || demoState.released}
              ...
              >
                {demoState.released ? "Complete" : "Mark Issue Complete"}
              </Button>
            ...
            <section className="rounded-premium border border-slate-200 bg-white p-5 shadow-card">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-2 text-xs leading-5 text-slate-500">The billing team still makes the final decision.</p>
                </div>
              </div>
            </section>
        ...
        <p className="mt-8 max-w-4xl text-xs leading-5 text-slate-500">
          {brand.disclaimer} This screen uses fictional sample data and browser-only state. It does not send messages, reroute trucks, confirm disposal, or make final billing decisions.
        </p>
      </main>
    </Layout>
  );
