...
function KpiRow({ mobile = false }) {
  return (
    <div className={mobile ? "flex snap-x gap-2 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" : "grid grid-cols-4 gap-3"}>
      {kpis.map((item) => (
...
function BoardCanvas({ mobile = false }) {
  ...
  return (
    <section className={`overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-premium ${mobile ? "min-w-[88vw] snap-center" : ""}`}>
      ...
      <div className={`relative ${mobile ? "h-[22rem]" : "h-[32rem]"} bg-[radial-gradient(circle_at_1px_1px,rgba(15,23,42,0.08)_1px,transparent_0)] [background-size:26px_26px]`}>
  ...
  return (
    <Link to={`/proof/${packet.id}`} data-testid={`proof-card-${packet.id}`} className={`${compact ? "min-w-[82vw] snap-start" : ""} group rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-card transition-all hover:-translate-y-0.5 hover:shadow-card-hover`}>
      ...
      <div className="mt-4 grid grid-cols-3 gap-2">
        {[
          ["Missing", packet.missingProofCount],
          ["Weak", packet.weakBackupCount],
  ...
  return (
    <div className="min-h-screen bg-offwhite pb-24 lg:hidden">
      ...
      <main className="px-3 py-4">
        ...
        <KpiRow mobile />
        <section className="mt-3 flex snap-x gap-3 overflow-x-auto pb-2 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          <BoardCanvas mobile />
