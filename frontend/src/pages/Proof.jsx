...
function DesktopRail() {
  return (
    <aside className="hidden w-20 shrink-0 rounded-3xl bg-navy-950 p-3 text-white shadow-premium lg:flex lg:flex-col lg:items-center">
      ...
      <div className="mt-8 flex flex-1 flex-col items-center gap-3">
        {navItems.map((item) => {
          ...
          return (
            <button key={item.label} type="button" aria-label={item.label} className={`flex h-11 w-11 items-center justify-center rounded-2xl ${item.active ? "bg-white text-navy-950" : "text-slate-300 hover:bg-white/10 hover:text-white"}`}>
...
function KpiRow({ mobile = false }) {
  const wrapperClass = mobile ? "flex gap-2 overflow-x-auto pb-1" : "grid grid-cols-4 gap-3";
  return (
    <div className={wrapperClass} tabIndex={mobile ? 0 : undefined} role={mobile ? "region" : undefined} aria-label={mobile ? "Dashboard metrics" : undefined}>
      {kpis.map((item) => (
...
function BoardNode({ packet }) {
  const config = getStatus(packet);
  const Icon = config.icon;
  return (
    <Link to={`/proof/${packet.id}`} className="rounded-2xl border border-slate-200 bg-white p-3 shadow-card">
      <div className="flex items-center gap-2">
        <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${config.tone}`}>
          <Icon className="h-4 w-4" aria-hidden="true" />
        </span>
        <div className="min-w-0">
          <p className="truncate text-sm font-semibold text-navy-950">{config.full}</p>
          <p className="text-xs font-medium text-slate-600">{packet.missingProofCount} missing · {packet.weakBackupCount} weak</p>
        </div>
      </div>
    </Link>
  );
...
function BoardCanvas({ mobile = false }) {
  return (
    <section className={`${mobile ? "w-[21rem] shrink-0" : ""} overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-premium`}>
      ...
      <div className="bg-offwhite p-4">
        <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-card">
          ...
          <div className="grid gap-3">
            {proofPackets.map((packet) => <BoardNode key={packet.id} packet={packet} />)}
          </div>
        </div>
      </div>
    </section>
...
function PacketCard({ packet, compact = false }) {
  ...
  const Icon = config.icon;
  return (
    <Link to={`/proof/${packet.id}`} data-testid={`proof-card-${packet.id}`} className={`${compact ? "w-72 shrink-0" : ""} rounded-3xl border border-slate-200 bg-white p-4 shadow-card hover:shadow-card-hover`}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 items-start gap-3">
          <span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl ${config.tone}`}>
            <Icon className="h-5 w-5" aria-hidden="true" />
          </span>
          <div className="min-w-0">
            <p className="truncate font-display text-base font-semibold text-navy-950">{config.full}</p>
    ...
    <div className="min-h-screen bg-offwhite pb-24 lg:hidden">
      ...
      <main className="px-3 py-4">
        ...
        <section className="mt-3 flex gap-3 overflow-x-auto pb-2" tabIndex={0} role="region" aria-label="Swipe dashboard panels">
          <BoardCanvas mobile />
          <div className="w-[21rem] shrink-0 rounded-3xl border border-slate-200 bg-white p-4 shadow-premium">
